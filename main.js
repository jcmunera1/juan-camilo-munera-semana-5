...
// Open up the calendar when clicked
date_picker_element.addEventListener("click", toggleDatePicker);
// Navigate through the calendar months when the arrows are clicked
  ...
  for (let i = 0; i < amount_days; i++) {
    ...
    day_element.addEventListener("click", function () {
      if (!selectedDate || selectedDateTwo) {
        ...
        selectedMonth = month;
        selectedYear = year;
        //It is done so the date is de-selected
...
function validateRange(currentDay) {
  if (
    ...
    currentDay + 1 > selectedDay &&
    currentDay + 1 < selectedDayTwo &&
    year >= selectedYear &&
    year <= selectedYearTwo &&
    month >= selectedMonth &&
    month <= selectedMonthTwo
  )
    return true;
  else if (
    selectedDate > selectedDateTwo &&
